function [combinations, branches] = gen_all_possibilities(M)
...
ultimo = M;
combinations = zeros(n_combinaciones, 4);
branches = zeros(M, dim_ramificaciones);
cont = ones(M,1);
i=1;
  ...
  for segundo = primero+1 : ultimo
      for tercero = primero+1 : ultimo
          ...
          for cuarto = tercero + 1 : ultimo
              ...
              combinations(i,:) = combinacion;
              branches(primero, cont(primero)) = i;
              branches(segundo, cont(segundo)) = i;
              branches(tercero, cont(tercero)) = i;
              branches(cuarto,  cont(cuarto )) = i;
